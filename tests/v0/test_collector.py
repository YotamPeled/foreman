...
def test_tick_records_roster_fields_and_derived_numbers(
    ...
    assert session["seconds_since_activity"] == 60.0
...
def test_job_stalled(env, fake_pool, children):
    """No mtime change and no cpu change for ten minutes marks the session
    stalled and opens one line."""
    write_config(base_config())
    proc = sleeper(children)
    sid = "ses-stal0001"
    FakeAdapter.script[sid] = {"transcript_mtime": 1700000000.0,
                               "cpu_s": 1.5,
                               "finish_present": False,
                               "finish_rc": None}
    seed_roster({sid: worker_session(sid, proc.pid, started_at=at(3600))})
...
def test_resolved_stall_closes_without_reopening(env, fake_pool, children):
    """Fresh activity resolves the stall line; staying active afterwards
    appends no second open line."""
    write_config(base_config())
    proc = sleeper(children)
    sid = "ses-stal0003"
    FakeAdapter.script[sid] = {"transcript_mtime": 1700000000.0,
    ...
    assert resolved[1]["resolved_at"] == iso(NOW)
    roster = json.loads(paths.roster_path().read_text(encoding="utf-8"))
...
def test_intruder_then_resolved(env, fake_pool, children):
    """A marker-matching process no session claims is flagged by pid; a
    dead pid clears the line instead of lingering."""
    write_config(base_config())
    proc = sleeper(children, MARKER)
    seed_roster({})
...
def test_claimed_processes_are_not_intruders(env, fake_pool, children):
    """A roster session's own tree, marker in its cmdline or not, never
...
def test_unregistered_writer(env, fake_pool, children):
    """A ledger line whose `by` the roster never minted opens one line;
    the owner's own lines never do."""
    write_config(base_config())
    seed_roster({})
    store.append_ledger(paths.inbox_path(), {
        "id": "inb-ghost", "from": "ses-ghost99", "kind": "error",
        "question": "q?", "recommendation": "r",
        "asked_at": at(10)}, session_id="ses-ghost99")
    store.append_ledger(paths.inbox_path(), {
    ...
    assert len(ghost) == 1
    assert "inbox.jsonl" in ghost[0]["detail"]
    assert lines_for("unregistered writer", "owner") == []


def test_dead_supervisor_relaunched_exactly_once(env, fake_pool, children):
    """A dead supervisor comes back as one new running session carrying
    the checkpoint, its slots released; later ticks add no second copy."""
    write_config(base_config())
    ...
    roster = json.loads(paths.roster_path().read_text(encoding="utf-8"))
    assert roster["sessions"][sid]["state"] == "exited"
    fresh = [rec for rec in roster["sessions"].values()
             if rec.get("role") == "supervisor"
             and rec.get("state") == "running"]
    assert len(fresh) == 1
    assert fresh[0]["id"] != sid
    assert fresh[0]["component"] == "comp"
    assert procs.pid_alive(fresh[0]["pid"])
    assert len(FakeAdapter.launched) == 1
    carried = json.loads(paths.checkpoint_path(
        fresh[0]["id"]).read_text(encoding="utf-8"))
    assert carried["doing"] == "split the brief"
...
def test_crash_loop_leaves_supervisor_dead(env, fake_pool, children):
    """Two relaunches inside the hour: the third death is left dead with
    an anomaly, and the adapter is never called."""
    write_config(base_config())
    old = sleeper(children)
    old.kill()
    old.wait()
...
def test_unknown_pool_is_left_alone(env, fake_pool, children):
    """A session naming a pool with no adapter keeps its state: without
    an observation source, unknown never reads as idle."""
    write_config(base_config())
    proc = sleeper(children)
    sid = "ses-nopool01"
    seed_roster({sid: worker_session(
