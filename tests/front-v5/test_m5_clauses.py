"""Milestone 5 clause files are listed by id 5.1 through 5.5."""
...
def test_milestone_5_list_prints_five_ids():
    proc = subprocess.run(
        [sys.executable, str(PROOF), "v5", "--milestone", "5", "--list"],
        cwd=str(REPO),
        env=_env(),
        capture_output=True,
        text=True,
    ...
    for line in proc.stdout.splitlines():
        ...
        ids.append(parts[1])
    assert tuple(sorted(ids)) == M5_IDS, proc.stdout
