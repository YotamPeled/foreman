"""Milestone 2 clause files are listed by id 2.1 through 2.5."""
...
def test_milestone_2_list_prints_five_ids():
    proc = subprocess.run(
        [sys.executable, str(PROOF), "v5", "--milestone", "2", "--list"],
        cwd=str(REPO),
        env=_env(),
        capture_output=True,
        text=True,
    ...
    for line in proc.stdout.splitlines():
        ...
        ids.append(parts[1])
    assert tuple(sorted(ids)) == M2_IDS, proc.stdout
