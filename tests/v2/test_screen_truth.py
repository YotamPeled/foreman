...
def test_launch_supervisor_writes_the_supervisor_on_the_record(
    ...
    monkeypatch.setattr(launch_module, "_confirm_started",
                        lambda pid: (777, None))
    # A windowed launch names its workspace since the launches task: with
    # no default_workspace in the config it is refused before anything is
    # written, which is that task's whole point.
    assert cli.main(["launch", "supervisor", "alpha",
                     "--workspace", "6"]) == 0
    out = capsys.readouterr().out
